...
pass: process.env.MAIL_PASS // ⚠️ Utiliser une variable d’environnement
}
});
...
// Tout ce qui est après /upload/vXXXXX/ constitue le public_id (sans extension)
const afterUpload = parts.slice(uploadIndex + 2).join('/');
return afterUpload.replace(/.[^/.]+$/, ''); // Supprime l’extension
}
...
resource_type: 'auto',
// FIX: split() retourne un tableau — on prend le nom sans l’extension
...
db.query(sqlCheck, [username, email], (errCheck, results) => {
    ...
    db.query(sql, [username, email, password], (err, result) => {
        if (err) {
            console.error(err);
            // FIX: res.json de succès était dans le bloc if(err) — corrigé
        ...
        // FIX: réponse de succès placée correctement après l'insertion
...
const mailOptions = {
from: '"SPE Congo" [ritakngot3@gmail.com](mailto:ritakngot3@gmail.com)',
to: emailMentor,
...
sousTitre: 'Programme de Mentorat SPE Congo',
contenu: `<p style="color:#1e293b; font-size:0.95rem; line-height:1.7;"> Cher(e) <strong>${nomMentor}</strong>, </p> <p style="color:#1e293b; font-size:0.95rem; line-height:1.7; margin-top:10px;"> Nous vous remercions chaleureusement d'avoir rejoint le programme de mentorat  de la <strong>SPE Congo</strong> en tant que mentor. </p> <p style="color:#1e293b; font-size:0.95rem; line-height:1.7; margin-top:10px;"> Votre expertise et votre expérience sont des atouts précieux pour accompagner  la nouvelle génération de professionnels de l'énergie au Congo. </p> <div style="background:#f0f9ff; border-left:4px solid #0054a6;  padding:15px 20px; border-radius:8px; margin-top:20px;"> <p style="margin:0 0 8px; color:#0054a6; font-weight:600; font-size:0.9rem;"> Quelle est la suite ? </p> <p style="margin:0; color:#1e293b; font-size:0.88rem; line-height:1.8;"> ✅ Notre équipe administrative va valider votre profil.<br> 📬 Nous reviendrons vers vous pour vous proposer un ou plusieurs mentees. </p> </div> <p style="color:#1e293b; font-size:0.95rem; line-height:1.7; margin-top:25px;"> Merci de contribuer au développement des compétences locales. </p> <p style="color:#64748b; font-size:0.85rem; margin-top:25px;"> Cordialement,<br> <strong>L'équipe de coordination SPE Congo</strong> </p>`
})
};
...
// FIX: ‘upload’ n’était pas défini — remplacé par uploadMentor
...
const mailOptions = {
from: '"SikaHub - SPE Congo" [ritakngot3@gmail.com](mailto:ritakngot3@gmail.com)',
...
titre: 'Bienvenue dans le programme !',
sousTitre: 'Programme de Mentorat SPE Congo',
contenu: `<p style="color:#1e293b; font-size:0.95rem; line-height:1.7;"> Félicitations <strong>${nomMentee}</strong> ! </p> <p style="color:#1e293b; font-size:0.95rem; line-height:1.7; margin-top:10px;"> Nous avons bien reçu ton inscription au programme de mentorat de la  <strong>Society of Petroleum Engineers (SPE) Congo</strong>. </p> <div style="background:#f0f9ff; border-left:4px solid #0054a6;  padding:15px 20px; border-radius:8px; margin-top:20px;"> <p style="margin:0 0 8px; color:#0054a6; font-weight:600; font-size:0.9rem;"> 📋 Prochaines étapes </p> <p style="margin:0; color:#1e293b; font-size:0.88rem; line-height:1.9;"> 1️⃣ Ton profil va être examiné par nos administrateurs.<br> 2️⃣ Tu peux consulter la liste des mentors disponibles selon ton domaine d'intérêt.<br> 3️⃣ Tu recevras une notification dès qu'un match sera effectué ! </p> </div> <p style="color:#1e293b; font-size:0.95rem; line-height:1.7; margin-top:25px;"> En attendant, n'hésite pas à consulter les dernières actualités sur notre plateforme. </p> <p style="color:#64748b; font-size:0.85rem; margin-top:25px;"> Cordialement,<br> <strong>L'équipe SPE Congo</strong> </p>`
})
};
return transporter.sendMail(mailOptions);
}
...
// Route mentee → demande mentor + envoi des mails
...
// Route pour récupérer les mentors (admin)
...
// Fonction mail décision candidature mentor
...
// Mettre à jour le statut du mentor après validation/refus et envoyer un mail
...
// Mettre à jour le statut du mentee
...
db.query("SELECT image_path, flyer_path FROM news WHERE id = ?", [id], (err, results) => {
    ...
    // FIX: results[0] au lieu de results
...
// Avec Cloudinary, le path contient l'URL complète
...
db.query("SELECT image_path FROM galerie WHERE id = ?", [photoId], (err, results) => {
    ...
    // FIX: results[0].image_path au lieu de results.image_path
